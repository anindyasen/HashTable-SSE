...
class alignas(64) HashMap
{
    //hash element array
    alignas(64) HashNode<K,V> **arr;
    int capacity;
    // no of group
    int num_groups_;
            ...
            if(arr[i] != NULL && arr[i]->key != -1) {
                 ...
                 int group_cnt = 0;
                  while(new_cntl_byte[pos + group_pos*16] != Empty && 
                        ...
                        new_arr[pos + group_pos*16]->key != arr[i]->key)) {
                      pos = (pos + 1) % 16;
                      hashSize++;
                      if(hashSize == 16) {
...
*/
    int get_cmp_key(uint8_t hash, __m128i *ctrl) {
        __m128i cmp_key = _mm_set1_epi8(hash);
        return _mm_movemask_epi8(_mm_cmpeq_epi8(cmp_key,*ctrl));
    }
    ...
    V get(int key)
    {
        ...
        while(1){
            int hashSize = 0;
            __m128i *ctrl = (__m128i*)(cntl_byte+(group_pos*16));
            int cmp_key = get_cmp_key(h2(hashIndex),ctrl);
            if(cmp_key!=0) {
                for (int i= 0;i<16; i++) {
                    if(1 == (cmp_key & 1) && arr[(i) + (group_pos*16)]->key == key) {
                    //if((NULL != arr[(i) + (group_pos*16)]) && (arr[(i) + (group_pos*16)]->key == key)) {
